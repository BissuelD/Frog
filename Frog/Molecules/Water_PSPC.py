...
def compute_mean_position(L_pos):
    ...
    '''
    vec_dipole = (L_pos[0]-L_pos[1])+(L_pos[2]-L_pos[1]) # ref centered around the oxygen atom
    vec_dipole = vec_dipole/np.sqrt(np.sum(vec_dipole**2))
    position_dipole = L_pos[1] + 0.1546*vec_dipole
...
def compute_hbonds(L_target, name_partner, L_partner, parameter, info = False):
    ...
    if name_partner in ['Water_PSPC']:
        ...
        L_H_water = [0, 2]
        index_oxygen = 1
        if np.sqrt(np.sum((L_target[index_oxygen] - L_partner[index_oxygen])**2)) <= parameter[0]:
        #Does the target molecule own an hbonds?
            for H_partner in L_H_water: 
                ...
                L_angle = np.array([L_partner[H_partner], L_target[index_oxygen], L_partner[index_oxygen]])
            ...
            for H_target in L_H_water: 
                ...
                L_angle = np.array([L_target[H_target], L_target[index_oxygen], L_partner[index_oxygen]])
                angle = geometry_manager.compute_angle_3_atoms(L_angle)
...
def compute_rotational_matrix(L_pos):
    ...
    '''
    vec_H1 = L_pos[0]-L_pos[1]
    vec_H2 = L_pos[2]-L_pos[1]
...
def electrostatic_description(pe_level, electro_description, L_pos=False):
    ...
    electro_description.L_localization_type = ['O', 'H', 'H','D']
    electro_description.L_localization_site = [L_pos[1], L_pos[0], L_pos[2], L_pos[3]]
    electro_description.L_charge_order_0 = [[1, -0.6690], [2, 0.3345], [3, 0.3345], [4, 0]] #Warning: contrarily to the rest of the code/python uses, the site are label from 1 (no 0!)
    
    if pe_level >= 1:
        electro_description.polarization_order = 1
        
        electro_description.L_polarization_order_1_1 = [[1, [9.72, 0.0, 0.0, 9.72, 0.0, 9.72]]] # polarizability diagonal: same value in the laboratory frame then in the molecular frame. 
...
def qm_target_description(qmparameter, qmdescription, L_pos=False):
    ...
    qmdescription.L_atom_type = [[8, 1, [['O', 1]]], [1, 2, [['H', 0], ['H', 2]]]]
